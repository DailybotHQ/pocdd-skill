...
> feature. It is **human-facing**: agents load the short operational contract in
> `skills/pocdd/shared/conventions.md` (generated by `./configure`), not this
> file, unless stuck or asked. Examples use generic placeholders (`<feature>`,
> `<provider>`, `<DomainModel>`). Track live POCs wherever your team tracks work
...
- **It owns the brain, not copies of the code.** The file owns goals, findings,
  ...
  codebase by path** (`path/to/file.ext:120`) — it does *not* paste production
  code into itself. It points; the agent opens the path when a step needs it.
  This is what keeps the file bounded no matter how large the feature is.
